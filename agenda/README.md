...
| 2 | Translator (SWE) | [`d2.md`](d2.md) | Harness (Bind) | **2–4** then **5–8** on ingest → landing | Type `01` translates safely, isolated. Bind is the fence. Consensus is the barrier before the first write |
...
| 2 | Recap Pass 0–1. Query Second Brain + OntoLayer (Java as concepts). **Bind the Agent Harness** (fail closed). ADRs, seams, **Consensus signed**, then Type `01` landing Parquet (`valid-minimal` publishes; refuses publish nothing). Research for Day 3. |
...
Structure → Decompose → Consensus, then the first modern write. The page
gate still holds: by Tuesday night the legacy is specified, structured, and
translated.
