"""Deterministic canonical Parquet publication.

Settings are pinned by DR-008. Statistics and dictionary encoding are disabled
so the file is a pure function of the rows and the schema, which makes the
Parquet SHA-256 itself a determinism check.

Publication is atomic: the data file, its checksum, and the readiness manifest
are written into a private temporary directory and renamed into place with the
manifest last, mirroring the legacy transport contract.
"""
...
def write_table(table: pa.Table, destination: Path) -> None:
    """Write one canonical Parquet file with every non-deterministic knob off."""
...
def canonical_metadata(
    *,
    ...
    """Return the immutable provenance embedded in the Parquet schema."""
    ...
    *,
    ...
    try:
        ...
        (staging / MANIFEST_NAME).write_text(
            json.dumps(
                {**dict(manifest), "parquet_sha256": digest},
                indent=2,
                sort_keys=True,
            )
            + "\n",
...
def decimal_column(values: Sequence[Any], precision: int = 18, scale: int = 2) -> pa.Array:
