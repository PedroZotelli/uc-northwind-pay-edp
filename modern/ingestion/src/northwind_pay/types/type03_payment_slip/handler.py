"""Compose admission, parsing, schema, and publication for one Type 03 batch."""
...
def process(bundle: Path, *, landing_root: Path) -> BatchOutcome:
    ...
    parquet_name = source.source_filename.replace(".rem", ".parquet")
    table = writer.table(
    ...
    result = publish(
        ...
        manifest={
            ...
            "computed_detail_count": controls.computed_logical_count,
            "computed_net_amount": f"{controls.computed_net_amount:.2f}",
            "contract_code": source.contract_code,
            "contract_version": source.contract_version,
            "currency": "BRL",
            "declared_detail_count": controls.declared_logical_count,
            "declared_net_amount": f"{controls.declared_net_amount:.2f}",
